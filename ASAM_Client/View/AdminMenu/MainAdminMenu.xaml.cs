...
namespace ASAM_Client.View.AdminMenu
{
    ...
    public partial class MainAdminMenu : Window
    {
        ...
        {
            var result = MessageBox.Show("Voulez vous éteindre ce PC ?","Eteindre",MessageBoxButton.YesNo,MessageBoxImage.Question);
        ...
        {
            var result = MessageBox.Show("Voulez vous quitter ASAM ?" + " Le client se relancera si le PC est redémarré.", "Quitter ASAM Client", MessageBoxButton.YesNo, MessageBoxImage.Question);
            if (result == MessageBoxResult.Yes)
            {
